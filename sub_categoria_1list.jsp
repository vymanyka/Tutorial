...
if (searchwhere != null && searchwhere.length() > 0) {
	session.setAttribute("sub_categoria_1_searchwhere", searchwhere);
	startRec = 1; // Reset start record counter (new search)
	session.setAttribute("sub_categoria_1_REC", new Integer(startRec));
}else{
	if (session.getAttribute("sub_categoria_1_searchwhere") != null)
		searchwhere = (String) session.getAttribute("sub_categoria_1_searchwhere");
}
%>
<%
...
if (OrderBy != null && OrderBy.length() > 0) {
	if (session.getAttribute("sub_categoria_1_OB") != null &&
		((String) session.getAttribute("sub_categoria_1_OB")).equals(OrderBy)) { // Check if an ASC/DESC toggle is required
		if (((String) session.getAttribute("sub_categoria_1_OT")).equals("ASC")) {
			session.setAttribute("sub_categoria_1_OT", "DESC");
		}else{
			session.setAttribute("sub_categoria_1_OT", "ASC");
		}
	}else{
		session.setAttribute("sub_categoria_1_OT", "ASC");
	}
	session.setAttribute("sub_categoria_1_OB", OrderBy);
	session.setAttribute("sub_categoria_1_REC", new Integer(1));
}else{
	OrderBy = (String) session.getAttribute("sub_categoria_1_OB");
	if (OrderBy == null || OrderBy.length() == 0) {
		OrderBy = DefaultOrder;
		session.setAttribute("sub_categoria_1_OB", OrderBy);
		session.setAttribute("sub_categoria_1_OT", DefaultOrderType);
	}
}
...
// Build SQL
String strsql = "SELECT * FROM `sub_categoria_1`";
whereClause = "";
...
if (OrderBy != null && OrderBy.length() > 0) {
	strsql = strsql + " ORDER BY `" + OrderBy + "` " + (String) session.getAttribute("sub_categoria_1_OT");
}
...
if (request.getParameter("start") != null && Integer.parseInt(request.getParameter("start")) > 0) {
	...
	if (IsNumeric(request.getParameter("pageno"))) {
		...
		startRec = ((Integer) session.getAttribute("sub_categoria_1_REC")).intValue();
		if (startRec <= 0) {
	...
	if (session.getAttribute("sub_categoria_1_REC") != null)
		startRec = ((Integer) session.getAttribute("sub_categoria_1_REC")).intValue();
	if (startRec==0) {
		startRec = 1; //Reset start record counter
		session.setAttribute("sub_categoria_1_REC", new Integer(startRec));
...
<p><span class="jspmaker">TABELA: Cat.2</span></p>
<form action="sub_categoria_1list.jsp">
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		...
		&nbsp;&nbsp;<a href="sub_categoria_1list.jsp?cmd=reset">Mostrar todos</a>
		&nbsp;&nbsp;<a href="sub_categoria_1srch.jsp">Busca avancada</a>
		</span></td>
	</tr>
...
<a href="sub_categoria_1list.jsp?order=<%= java.net.URLEncoder.encode("Descricao_da_categoria","UTF-8") %>" style="color: #FFFFFF;">Descricao da categoria&nbsp;(*)<% if (OrderBy != null && OrderBy.equals("Descricao_da_categoria")) { %><span class="ewTableOrderIndicator"><% if (((String) session.getAttribute("sub_categoria_1_OT")).equals("ASC")) {%>5<% }else if (((String) session.getAttribute("sub_categoria_1_OT")).equals("DESC")) { %>6<% } %></span><% } %></a>
		</span></td>
...
<% if ((ewCurSec & ewAllowView) == ewAllowView ) { %>
<td><span class="jspmaker"><a href="<% key =  rs.getString("id_Um"); 
if (key != null && key.length() > 0) { 
	out.print("sub_categoria_1view.jsp?key=" + java.net.URLEncoder.encode(key,"UTF-8"));
}else{
...
<td><span class="jspmaker"><a href="<% key =  rs.getString("id_Um"); 
if (key != null && key.length() > 0) { 
	out.print("sub_categoria_1edit.jsp?key=" + java.net.URLEncoder.encode(key,"UTF-8"));
}else{
	out.print("javascript:alert('Registro invalido! A chave eh nula');");
} %>">Editar</a></span></td>
<% } %>
...
<td><span class="jspmaker"><a href="<% key =  rs.getString("id_Um"); 
if (key != null && key.length() > 0) { 
	out.print("sub_categoria_1add.jsp?key=" + java.net.URLEncoder.encode(key,"UTF-8"));
}else{
	out.print("javascript:alert('Registro invalido! A chave eh nula');");
} %>">Copiar</a></span></td>
<% } %>
...
<td><span class="jspmaker"><a href="<% key =  rs.getString("id_Um"); 
if (key != null && key.length() > 0) { 
	out.print("sub_categoria_1delete.jsp?key=" + java.net.URLEncoder.encode(key,"UTF-8"));
}else{
	out.print("javascript:alert('Registro invalido! A chave eh nula');");
} %>">Excluir</a></span></td>
<% } %>
...
<!--first page button-->
	...
	<% }else{ %>
	<td><a href="sub_categoria_1list.jsp?start=1"><img src="images/first.gif" alt="First" width="20" height="15" border="0"></a></td>
	<% } %>
<!--previous page button-->
	<% if (PrevStart == startRec) { %>
	<td><img src="images/prevdisab.gif" alt="Previous" width="20" height="15" border="0"></td>
	<% }else{ %>
	<td><a href="sub_categoria_1list.jsp?start=<%=PrevStart%>"><img src="images/prev.gif" alt="Previous" width="20" height="15" border="0"></a></td>
	<% } %>
<!--current page number-->
...
<!--next page button-->
	...
	<% }else{ %>
	<td><a href="sub_categoria_1list.jsp?start=<%=NextStart%>"><img src="images/next.gif" alt="Next" width="20" height="15" border="0"></a></td>
	<% } %>
<!--last page button-->
	<% if (LastStart == startRec) { %>
	<td><img src="images/lastdisab.gif" alt="Last" width="20" height="15" border="0"></td>
	<% }else{ %>
	<td><a href="sub_categoria_1list.jsp?start=<%=LastStart%>"><img src="images/last.gif" alt="Last" width="20" height="15" border="0"></a></td>
	<% } %>
<% if ((ewCurSec & ewAllowAdd) == ewAllowAdd) { %>
	<td><a href="sub_categoria_1add.jsp"><img src="images/addnew.gif" alt="Add new" width="20" height="15" border="0"></a></td>
<% } %>
...
<p>
<% if ((ewCurSec & ewAllowAdd) == ewAllowAdd) { %>
<a href="sub_categoria_1add.jsp"><img src="images/addnew.gif" alt="Add new" width="20" height="15" border="0"></a>
<% } %>
</p>
<% } %>
</td></tr></table>

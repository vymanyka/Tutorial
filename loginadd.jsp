...
try{
	...
	if (a.equals("C")){ // Get a record to display
    	...
    	if (userLevel != null && userLevel.intValue() != -1) { // Non system admin
			rs.updateString("Login", (String) session.getAttribute("sighs_status_UserID"));
		}
	...
	if (rs.getString("eMail") != null){
		...
		// Open record
		String strsql = "SELECT * FROM `login` WHERE 0 = 1";
		rs = stmt.executeQuery(strsql);
		rs.moveToInsertRow();
		...
		String srchfld = "'" + tmpfld + "'";
			...
			strsql = "SELECT * FROM `login` WHERE `Login` = '" + srchfld +"'";
			...
			if (rschk.next()) {
				out.print("Duplicar chave para Login, valor = " + tmpfld + "<br>");
...
<p><span class="jspmaker">Adicionar TABELA: Login<br><br><a href="loginlist.jsp">Voltar a lista</a></span></p>
...
<form onSubmit="return EW_checkMyForm(this);"  action="loginadd.jsp" method="post">
<p>

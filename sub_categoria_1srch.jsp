...
if ((ewCurSec & ewAllowSearch) != ewAllowSearch) {
	response.sendRedirect("sub_categoria_1list.jsp"); 
	response.flushBuffer(); 
	return;
}
%>
...
if (a != null && a.equals("S")) { // Get Search Criteria
	...
	if (search_criteria.length() > 0) {
		out.clear();
		response.sendRedirect("sub_categoria_1list.jsp" + "?" + search_criteria);
		response.flushBuffer();
		return;
	}
}
...
<p><span class="jspmaker">Procurar TABELA: Cat.2<br><br><a href="sub_categoria_1list.jsp">Voltar a lista</a></span></p>
<script language="JavaScript" src="ew.js"></script>
<script language="JavaScript">
<!-- start Javascript
function  EW_checkMyForm(EW_this) {
if (EW_this.x_id_Um && !EW_checkinteger(EW_this.x_id_Um.value)) {
        if (!EW_onError(EW_this, EW_this.x_id_Um, "TEXT", "Numero inteiro invalido! - Codigo"))
            return false; 
        }
return true;
}
...
<form onSubmit="return EW_checkMyForm(this);"  action="sub_categoria_1srch.jsp" method="post">
<p>
...
<input type="hidden" name="z_id_Um" value="=, , ">=
		...
		<td bgcolor="#F5F5F5"><span class="jspmaker"><input type="text" name="x_id_Um" value="<%= HTMLEncode((String)x_id_Um) %>"></span>&nbsp;</td>
	</tr>
	<tr>
